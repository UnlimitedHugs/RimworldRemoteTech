...
namespace RemoteExplosives {
	...
	public class CompRoofBreakerExplosive : CompMiningExplosive {
		...
		protected override void Detonate() {
			...
			var canAffectThickRoof = RemoteExplosivesUtility.IsEffectiveRoofBreakerPlacement(explosiveProps.explosiveRadius, parent.Position, map);
			foreach (var cell in GenRadial.RadialCellsAround(parent.Position, explosiveProps.explosiveRadius, true)) {
				...
				if (!roof.isThickRoof) {
					...
					if(!reservedRoofTiles.Contains(cell)) map.roofGrid.SetRoof(cell, null);
				} else {
					reservedRoofTiles.Add(cell);
					map.roofGrid.SetRoof(cell, RoofDefOf.RoofRockThin); // downgrade roof, so that charges can be chained
					var roofCell = cell;
					HugsLibController.Instance.CallbackScheduler.ScheduleCallback(() => { // delay collapse for more interesting visual effect
						...
						map.roofGrid.SetRoof(roofCell, roof); // plug thick roof back in so it can collapse
						RoofCollapserImmediate.DropRoofInCells(roofCell, map);
						map.roofGrid.SetRoof(roofCell, RoofDefOf.RoofRockThin); // place thin roof over collapsed rock to prevent roof build areas to be auto-generated by AutoBuildRoofZoneSetter
						map.areaManager.NoRoof[roofCell] = true;
						map.areaManager.BuildRoof[roofCell] = false;
						CaveInSoundEffect.PlayOneShot(new TargetInfo(roofCell, map));
